...
class UserController extends Controller
{
    public function userDashboard(Request $request)
    {
        ...
        $franchiseCategories = FranchiseCategory::latest()
            ->take(3)
            ->get();
        ...
        return view(
            'dashboard',
            compact('franchiseCategories', 'franchises', 'latestNews')
        )->with('successData', $successData);
    }
    ...
    public function update(Request $req)
    {
        ...
        if ($req->file('profileImage')) {
            ...
            Image::make($image)
                ->resize(430, 327)
                ->save(public_path($directory . $name_gen));
        ...
        $response = [
            'message' =>
                'Profile update successfully, please wait for approval',
    ...
    public function UpdatePassword(Request $request)
    {
        ...
        if ($user->password == "") {
            ...
            return redirect()
                ->route('dashboard')
                ->with('password_change_success', true);
        } else {
            $validator = $request->validate(
                [
    ...
    public function AdminDashboard()
    {
        ...
        //calculate user
        $currentUser = User::whereMonth(
            'created_at',
            Carbon::now()->month
        )->count();
        $previousMonthUser = User::whereMonth(
            'created_at',
        ...
        $currentFranchisor = User::where('role', 'Franchisor')
            ->whereMonth('created_at', Carbon::now()->month)
            ->count();
        $previousMonthFranchisor = User::where('role', 'Franchisor')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->count();
        $franchisorDifference = $currentFranchisor - $previousMonthFranchisor;

        //calculate franchise
        $currentFranchise = Franchise::whereMonth(
            'created_at',
            Carbon::now()->month
        )->count();
        $previousMonthFranchise = Franchise::whereMonth(
            'created_at',
            Carbon::now()->subMonth()->month
        )->count();
        ...
        //calculate education
        $currentEducation = EducationContent::whereMonth(
            'created_at',
            Carbon::now()->month
        )->count();
        $previousMonthEducation = EducationContent::whereMonth(
            'created_at',
            Carbon::now()->subMonth()->month
        )->count();
    ...
    public function calculateMonthlyUserIncrease()
    {
        ...
        $percentageIncrease =
            $previousMonthUsers > 0
                ? ($increase / $previousMonthUsers) * 100
                : 0;

        // Tampilkan atau gunakan nilai persentase peningkatan
        return "Persentase Peningkatan Pengguna dari Bulan Lalu: " .
